...
public sealed class GenericDataModel : AbstractDataModel {
  ...
  public override void RemovePreference(long userID, long itemID) {
            if (!this.preferenceFromUsers.ContainsKey(userID))
                throw new NoSuchUserException();

            //int size = preferenceFromUsers.Count();
            ////long userID = Int64.MinValue;
            //foreach (var user in preferenceFromUsers.EntrySet())
            //{
            //    if(user.Key==userID)
            ...
            //for (int i = 0; i < size; i++)
            //{
            //    var prefList = preferenceFromUsers.EntrySet();
            //    IPreference pref = (IPreference)prefList.ToList()[i];
            //    if (i == 0)
            //    {
            ...
            //var newEntrySet=new ;
            //var entrySet=this.preferenceFromUsers.Get(userID);
            //entrySet
            //foreach (var item in entrySet)
            //{
            //    newEntrySet.Set(i, item);
            //}
            


        //return new GenericDataModel(GenericDataModel.ToDataMap(preferenceFromUsers, preferenceForItems));
        }
