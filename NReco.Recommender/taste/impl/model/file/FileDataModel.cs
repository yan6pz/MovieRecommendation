...
public class FileDataModel : AbstractDataModel {
  ...
  public FileDataModel(string dataFile, bool transpose, long minReloadIntervalMS, bool uniqueUserItemCheck = true, string delimiterRegex = null,long userId=-1,long itemId=-1) {
  ...
  protected void reload(long userId,long itemId) {
    if (Monitor.TryEnter(this)) {
      try {
        _delegate = buildModel(userId,itemId);
...
protected IDataModel buildModel(long userId,long itemId) {
    ...
    if (hasPrefValues) {
      ...
      if (loadFreshData) {
        ...
        return new GenericDataModel(GenericDataModel.ToDataMap(data,userId,itemId, true), timestamps);
